...
common_fips_last <- Reduce(intersect, list(drought$fips, fire_processed_last$fips, population$fips, soil$fips, weather$fips))
...
#remove population and cultivate land, overdispersion, using Gamma 
...
residuals <- residuals(fit_3_weighted)
...
summary(fit_9,digits=2)
residuals <- residuals(fit_8)
plot(time_data$times, residuals)
abline(h=0)
post_resid <- posterior_predict(fit_9)
ppc_dens_overlay(time_data$times,post_resid[1:100,])
fit_10<- stan_glmer(times~drought_value+CENSUS2010POP*CULT_LAND+FOR_LAND+mean_QV2M+mean_WS10M+mean_WS50M+mean_PRECTOT + (1|STNAME),data=time_data, family=Gamma(link="inverse"))
```
### no pooling
